...
static struct snd_soc_codec *es8374 = NULL;
static int codec_bias_level = 0;
int es8374_jack_insert;
extern void es8316_mono_en(int enable);

static int es8374_jack_status_check(void);
...
static struct snd_soc_jack_gpio jack_gpio = {
	...
	.debounce_time	= 200,
	.jack_status_check = es8374_jack_status_check,
};
...
static int es8374_jack_status_check(void)
{
	struct snd_soc_codec *codec = es8374;
	...
	pr_debug("%s: jack_insert %d\n", __func__, es8374_jack_insert);

	return level;
}

static int es8374_hw_params(struct snd_pcm_substream *substream,
...
static int es8374_suspend_pre(struct snd_soc_card *card)
{
    printk("+%s\n", __func__);
...
static int es8374_resume_pre(struct snd_soc_card *card)
{
...
static int es8374_resume_post(struct snd_soc_card *card)
{
    struct snd_soc_codec *codec = es8374;
   	int jack = jack_gpio.gpio;
	int invert = jack_gpio.invert;
...
static struct snd_soc_ops es8374_ops = {
	.hw_params 		= es8374_hw_params,
};

int es8374_spk_on(int enable)
{
	pr_debug("%s (enable:%d)\n", __func__, enable);
#if defined (CFG_IO_AUDIO_AMP_POWER)
	if (enable)
...
EXPORT_SYMBOL(es8374_spk_on);

static int es8374_spk_event(struct snd_soc_dapm_widget *w,
...
static const struct snd_soc_dapm_route es8374_audio_map[] = {
	/* headphone connected to HPOL, HPOR */
	{"Headphone Jack", NULL, "DIFFERENTIAL OUT"},

	/* speaker connected to HPOL, HPOR */
	{"Ext Spk", NULL, "DIFFERENTIAL OUT"},
};
...
static int es8374_dai_init(struct snd_soc_pcm_runtime *rtd)
{
...
static struct snd_soc_dai_link es8374_dai_link = {
	.name 		= "ASOC-ES7243E",
	.stream_name 	= "es7243e HiFi",
	.cpu_dai_name 	= str_dai_name,			/* nxp_snd_i2s_driver name */
	.platform_name  = DEV_NAME_PCM,			/* nxp_snd_pcm_driver name */
	.codec_dai_name = "ES7243E HiFi 0",		/* es8316_dai's name */
	.codec_name 	= "es7243e.0-0013",		/* es8316_i2c_driver name + '.' + bus + '-' + address(7bit) */
	.ops 			= &es8374_ops,
	.symmetric_rates = 1,
	.init		= es8374_dai_init,
};

static struct snd_soc_card es8374_card = {
	.name 				= "I2S-ES7243E",		/* proc/asound/cards */
	.owner 				= THIS_MODULE,
	.dai_link 			= &es8374_dai_link,
	.num_links 			= 1,
	//.suspend_pre		= &es8374_suspend_pre,
	//.resume_pre		= &es8374_resume_pre,
	//.resume_post		= &es8374_resume_post,
	.dapm_widgets 		= es8374_dapm_widgets,
	.num_dapm_widgets 	= ARRAY_SIZE(es8374_dapm_widgets),
	.dapm_routes 		= es8374_audio_map,
	.num_dapm_routes 	= ARRAY_SIZE(es8374_audio_map),
};

/*
 * codec driver
 */
static int es8374_platform_probe(struct platform_device *pdev)
{
	struct nxp_snd_dai_plat_data *plat = pdev->dev.platform_data;
	struct snd_soc_card *card = &es8374_card;
	struct snd_soc_jack_gpio *jack = &jack_gpio;
	...
	if (plat) {
		...
		if (hpin->support) {
			...
			jack->name = NULL;
			es8374_jack_insert = 1;	/* Stereo out */
		}
	}
#if 1
#if defined (CFG_IO_AUDIO_AMP_POWER)
	...
	printk("Enter into %s, register snd card!\n", __func__);
	card->dev = &pdev->dev;
	...
	printk("Enter into %s, get i2s dai\n", __func__);
	if (card->rtd) {
		struct snd_soc_dai *cpu_dai = card->rtd->cpu_dai;
		if (cpu_dai)
			i2s_dai = cpu_dai->driver;
	}
	printk("es8374-dai: register card %s -> %s\n",
...
static int es8374_platform_remove(struct platform_device *pdev)
{
...
static struct platform_driver es8374_platform_driver = {
	.driver		= {
		.name	= "es7243e-audio",
		.owner	= THIS_MODULE,
		.pm 	= &snd_soc_pm_ops,	/* for suspend */
	},
	.probe		= es8374_platform_probe,
	.remove		= __devexit_p(es8374_platform_remove),
};
module_platform_driver(es8374_platform_driver);

MODULE_AUTHOR("hsjung <hsjung@nexell.co.kr>");
MODULE_DESCRIPTION("Sound codec-es8311 driver for the SLSI");
MODULE_LICENSE("GPL");
