...
static struct snd_soc_codec *es7210 = NULL;
#if 0
...
#endif
static struct snd_soc_ops es7210_ops = {
...
#endif
static struct snd_soc_dai_link es7210_dai_link = {
	.name 			= "ASOC-ES7210",
	.stream_name 	= "es7210 HiFi",
	.cpu_dai_name 	= str_dai_name,			/* nxp_snd_i2s_driver name */
	.platform_name  = DEV_NAME_PCM,			/* nxp_snd_pcm_driver name */
	.codec_dai_name = "ES7210 HiFi",		/* es8316_dai's name */
	.codec_name 	= "ES7210.0-0040",		/* es8316_i2c_driver name + '.' + bus + '-' + address(7bit) */
	.ops 			= &es7210_ops,
	.symmetric_rates = 1,
	//.init		= es8316_dai_init,
};

static struct snd_soc_card es7210_card = {
	.name 				= "I2S-ES7210",		/* proc/asound/cards */
	.owner 				= THIS_MODULE,
	.dai_link 			= &es7210_dai_link,
	.num_links 			= 1,
 ...
 * codec driver
 */
static int es7210_probe(struct platform_device *pdev)
{
	struct nxp_snd_dai_plat_data *plat = pdev->dev.platform_data;
	struct snd_soc_card *card = &es7210_card;
	//struct snd_soc_jack_gpio *jack = &jack_gpio;
	...
	pr_debug("es7210-dai: register card %s -> %s\n",
...
static int es7210_remove(struct platform_device *pdev)
{
...
static struct platform_driver es7210_driver = {
	.driver		= {
		.name	= "es7210-audio",
		.owner	= THIS_MODULE,
	...
	.probe		= es7210_probe,
	.remove		= __devexit_p(es7210_remove),
};
module_platform_driver(es7210_driver);

MODULE_AUTHOR("hsjung <hsjung@nexell.co.kr>");
MODULE_DESCRIPTION("Sound adc-es7210 driver for the ALSA");
MODULE_LICENSE("GPL");
