...
static struct snd_soc_codec *es7243 = NULL;
#if 0
...
#endif
static struct snd_soc_ops es7243_ops = {
...
#endif
static struct snd_soc_dai_link es7243_dai_link = {
	.name 			= "ASOC-ES7243",
	.stream_name 	= "es7243 HiFi",
	.cpu_dai_name 	= str_dai_name,			/* nxp_snd_i2s_driver name */
	.platform_name  = DEV_NAME_PCM,			/* nxp_snd_pcm_driver name */
	.codec_dai_name = "ES7243 HiFi",		/* es8316_dai's name */
	.codec_name 	= "ES7243.0-0040",		/* es8316_i2c_driver name + '.' + bus + '-' + address(7bit) */
	.ops 			= &es7243_ops,
	.symmetric_rates = 1,
	//.init		= es8316_dai_init,
};

static struct snd_soc_card es7243_card = {
	.name 				= "I2S-ES7243",		/* proc/asound/cards */
	.owner 				= THIS_MODULE,
	.dai_link 			= &es7243_dai_link,
	.num_links 			= 1,
 ...
 * codec driver
 */
static int es7243_probe(struct platform_device *pdev)
{
	struct nxp_snd_dai_plat_data *plat = pdev->dev.platform_data;
	struct snd_soc_card *card = &es7243_card;
	//struct snd_soc_jack_gpio *jack = &jack_gpio;
...
static int es7243_remove(struct platform_device *pdev)
{
...
static struct platform_driver es7243_driver = {
	.driver		= {
		.name	= "es7243-audio",
		.owner	= THIS_MODULE,
	...
	.probe		= es7243_probe,
	.remove		= __devexit_p(es7243_remove),
};
module_platform_driver(es7243_driver);

MODULE_AUTHOR("hsjung <hsjung@nexell.co.kr>");
MODULE_DESCRIPTION("Sound adc-es7243 driver for the ALSA");
MODULE_LICENSE("GPL");
