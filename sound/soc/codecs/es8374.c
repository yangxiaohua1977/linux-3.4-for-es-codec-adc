...
struct snd_soc_codec *es8374_codec;
...
static const struct snd_kcontrol_new es8374_speaker_mixer_controls[] = {
	SOC_DAPM_SINGLE("LIN TO SPEAKER OUT Switch", ES8374_SPK_MIX_REG1C, 6, 1, 1),
	SOC_DAPM_SINGLE("DAC TO SPEAKER OUT Switch", ES8374_SPK_MIX_REG1C, 7, 1, 1),
};
/*
...
static int es8374_set_dai_fmt(struct snd_soc_dai *codec_dai, unsigned int fmt)
{
	...
	case SND_SOC_DAIFMT_CBM_CFM:    /* MASTER MODE */
		printk("ES8374 in Master mode\n");
		iface |= 0x80;
		break;
	case SND_SOC_DAIFMT_CBS_CFS:    /* SLAVE MODE */
		printk("ES8374 in Slave mode\n");
		iface |= 0x80;
		iface &= 0x7F;
		break;
	default:
...
//	struct es8374_private *es8374 = snd_soc_codec_get_drvdata(codec);
	...
	case SND_SOC_BIAS_PREPARE:
                ...
                snd_soc_write(codec, ES8374_ANA_REF_REG14, 0x8a);
                snd_soc_write(codec, ES8374_ANA_PWR_CTL_REG15, 0x40);
		snd_soc_write(codec, ES8374_AIN_PWR_SRC_REG21, 0x14);
		snd_soc_write(codec, ES8374_MONO_MIX_REG1A, 0xa0);
                snd_soc_write(codec, ES8374_SPK_MIX_REG1C, 0x90);
                snd_soc_write(codec, ES8374_SPK_MIX_GAIN_REG1D, 0x02);
                snd_soc_write(codec, ES8374_SPK_OUT_GAIN_REG1E, 0xa0);
...
*/
	...
	snd_soc_write(codec,0x10,0x0C);	//ADC I2S-16BIT
	snd_soc_write(codec,0x11,0x0C);	//DAC I2S-16BIT
	...
	snd_soc_write(codec,0x22,0x55);	//adc set:	PGA GAIN=0DB
	snd_soc_write(codec,0x21,0x14);	//adc set:	SEL LIN1 CH+PGAGAIN=0DB
...
static ssize_t es8374_show(struct device *dev, struct device_attribute *attr, char *_buf)
{
                int ret;
                ret = sprintf(_buf, "%s(): get 0x%04x=0x%04x\n", __FUNCTION__, cur_reg,
                snd_soc_read(es8374_codec, cur_reg));
...
static ssize_t es8374_store(struct device *dev,
                                        ...
                                        const char *buf, size_t count)
{
	...
	//struct es7210_priv *es7210 = dev_get_drvdata(dev);
        ...
        if (flag) {
                ...
                while(flag--) {
                        snd_soc_write(es8374_codec, reg, value_w);
                ...
                do {
                        value_r = 0;
                        value_r = snd_soc_read(es8374_codec, reg);
...
static struct device *es8374_dev = NULL;
static struct class *es8374_class = NULL;
static DEVICE_ATTR(es8374, 0664, es8374_show, es8374_store);

static struct attribute *es8374_debug_attrs[] = {
        &dev_attr_es8374.attr,
        NULL,
};

static struct attribute_group es8374_debug_attr_group = {
        .name   = "es8374_debug",
        .attrs  = es8374_debug_attrs,
};
		...
		const struct i2c_device_id *id)
{
	...
	int ret = -1;
	printk("Enter into %s\n", __func__);
	es8374 = devm_kzalloc(&i2c_client->dev, sizeof(*es8374), GFP_KERNEL);
	if (es8374 == NULL)
		return -ENOMEM;
...
#if 0        
	es8374_class = class_create(THIS_MODULE, "es8374");
        if (IS_ERR(es8374_class))
        {
                printk("Create class audio_es8374.\n");
                return -ENOMEM;
        }
	
	printk("Enter into %s------5\n", __func__);
        es8374_dev = device_create(es8374_class, NULL, MKDEV(0, 1), NULL, "dev");
        ret = device_create_file(es8374_dev, &dev_attr_es8374);
        if (ret < 0)
                printk("failed to add dev_attr_es8374 file\n");
#endif
        ret = sysfs_create_group(&i2c_client->dev.kobj, &es8374_debug_attr_group);
        if (ret) {
...
//module_i2c_driver(es8374_i2c_driver);
