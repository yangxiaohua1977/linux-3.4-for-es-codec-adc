...
static int es7243e_write(u8 reg, unsigned char value, struct i2c_client *client)
{
	int ret = 0;
	u8 write_cmd[2] = {0};
...
static int es7243e_update_bits(u8 reg, u8 mask, u8 value, struct i2c_client *client)
{
	u8 val_old,val_new;
...
*/
static int es7243e_set_dai_sysclk(struct snd_soc_dai *codec_dai,
...
static int es7243e_set_dai_fmt(struct snd_soc_dai *codec_dai,
		unsigned int fmt)
{
    //struct snd_soc_codec *codec = codec_dai->codec;
    u8 iface = 0;
...
{ 
	...
	*/
	if(coeff >= 0)
	{
		...
		{
			es7243e_write(0x03, coeff_div[coeff].osr, i2c_clt[index]);
			es7243e_write(0x04, coeff_div[coeff].prediv_premulti, i2c_clt[index]);
			es7243e_write(0x05, coeff_div[coeff].cf_dsp_div, i2c_clt[index]);
			es7243e_write(0x0d, coeff_div[coeff].scale, i2c_clt[index]);
			es7243e_write(0x03, coeff_div[coeff].osr, i2c_clt[index]);
			...
			es7243e_write(0x06, coeff_div[coeff].bclkdiv, i2c_clt[index]);
...
static int es7243e_set_bias_level(struct snd_soc_codec *codec,
...
#define es7243e_RATES SNDRV_PCM_RATE_8000_96000
...
static inline int get_mclk_lrck_ratio(int clk_ratio, int n_fs)
{
	...
	for (i = 0; i < ARRAY_SIZE(ratio_div); i++) {
		if (ratio_div[i].ratio == clk_ratio && ratio_div[i].nfs == n_fs)
			return i;
	}
...
static int es7243e_probe(struct snd_soc_codec *codec)
{
...
#else
	codec->control_data = devm_regmap_init_i2c(es7243e->i2c, &es7243e_regmap_config);
	...
	for(index=0; index<(ES7243E_CHANNELS_MAX)/2; index++)
	{
		...
		*/	
		...
		if(ratio_index < 0)
		{
			printk("can't get configuration for %d ratio with %d es7243e",
				ratio, work_mode);
		...
		{
			es7243e_write(0x03, ratio_div[ratio_index].osr, i2c_clt[index]);
			es7243e_write(0x0d, ratio_div[ratio_index].scale, i2c_clt[index]);
			es7243e_write(0x04, ratio_div[ratio_index].prediv_premulti, i2c_clt[index]);
			es7243e_write(0x05, ratio_div[ratio_index].cf_dsp_div, i2c_clt[index]);
		}
		...
		es7243e_write(0x18, 0x04, i2c_clt[index]);
		...
		es7243e_write(0x20, (0x10 | pga_gain[index*2]), i2c_clt[index]);
		es7243e_write(0x21, (0x10 | pga_gain[index*2 + 1]), i2c_clt[index]);
...
static ssize_t es7243e_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int val=0, flag=0;
...
static ssize_t es7243e_show(struct device *dev, struct device_attribute *attr, char *buf)
{
				   ...
				   const struct i2c_device_id *i2c_id)
{
	...
	es7243e = devm_kzalloc(&i2c->dev, sizeof(struct es7243e_priv), GFP_KERNEL);
	if (es7243e == NULL)
		return -ENOMEM;
  	es7243e->i2c = i2c;
	...
	if (i2c_id->driver_data < (ES7243E_CHANNELS_MAX)/2) {
		...
		ret = snd_soc_register_codec(&i2c->dev,&soc_codec_dev_es7243e, 
						es7243e_dai[i2c_id->driver_data], 1);
		if (ret < 0) {
			kfree(es7243e);
			return ret;
		}
...
static int es7243e_i2c_detect(struct i2c_client *client, struct i2c_board_info *info)
{
