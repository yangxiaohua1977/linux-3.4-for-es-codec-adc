 ...
 */
static u16 es8396_reg_defaults[] = {
	0x00, 0x00, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x50, 0x04, 0x00, 0x20, 0x20, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00,	0x00, 0x00, 0x00, 0x00,
 	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x11, 0x00, 
	0x00, 0x04, 0x00, 0x33, 0x00, 0x04, 0x00, 0x11,
	0x00, 0x04, 0x00, 0x11, 0x00, 0x04, 0x00, 0x11,
	0x00, 0x04, 0x44, 0x00, 0x20, 0x00, 0x00, 0x00,
	0x08, 0x88, 0x20, 0x82, 0x03, 0xa0, 0x00, 0x00,
	0x01, 0x01, 0x80, 0x80, 0x80, 0x80, 0x84, 0x84,
	0x84, 0x84, 0xc0, 0x80, 0x00, 0x00, 0xc0, 0xc0,
	0x0b, 0x32, 0x00, 0x1f, 0xc0, 0x00, 0xfc, 0x02,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80, 0x00,
	0x00, 0x00, 0xc0, 0xc0, 0x00, 0x00, 0xc8, 0x00,
	0xd3, 0x90, 0x00, 0x00, 0x88, 0xc1, 0x00, 0x00,
};
...
static void init_cali_work_events(struct work_struct *work)
{
	...
	//	snd_soc_write(tron_codec, ES8396_DAC_OFFSET_CALI_REG6F, 0x03);
...
static void voice_pop_work_events(struct work_struct *work)
{
	...
	snd_soc_update_bits(tron_codec, ES8396_SDP2_OUT_FMT_REG23,
			    0x7F, 0x33);
#if 1  //use line out
	//	snd_soc_write(tron_codec, 0x09, 0x08);
		snd_soc_write(tron_codec, 0x4E, 0x80);
		snd_soc_write(tron_codec, 0x4F, 0x81);	
		snd_soc_write(tron_codec, 0x4A, 0x60);
		snd_soc_write(tron_codec, 0x4B, 0x60);				
				 ...
				 enum snd_soc_bias_level level)
{
	...
	//	snd_soc_write(codec, 0x4F, 0x85);	
        //	snd_soc_write(codec, 0x71, 0xff);
	//	snd_soc_write(codec, 0x4a, 0x80);	
	//	snd_soc_write(codec, 0x4b, 0x80);	
	//	snd_soc_write(codec, 0x52, 0xc1);	
			/* cphp pdn */
...
static int es8396_probe(struct snd_soc_codec *codec)
{
	...
	//schedule_delayed_work(&u_es8396->init_cali_work, msecs_to_jiffies(50));
	/*
...
static int es8396_remove(struct snd_soc_codec *codec)
{
	...
	//es8396_set_bias_level(codec, SND_SOC_BIAS_OFF);
        ...
        snd_soc_write(codec, 0X4B, 0x80);
        #if 0
	snd_soc_write(codec, 0X72, 0x00);
        snd_soc_write(codec, 0X73, 0x00);
        snd_soc_write(codec, 0X4E, 0x84);
        snd_soc_write(codec, 0X4F, 0x85);
	snd_soc_write(codec, 0x71, 0xfc);		
        snd_soc_write(codec, 0X4A, 0x80);
        snd_soc_write(codec, 0X4B, 0x80);
        snd_soc_write(codec, 0X52, 0xc1);
        ...
        //usleep_range(20000, 50000);
	msleep(300);
        //snd_soc_write(codec, 0X4B, 0x00);
        //snd_soc_write(codec, 0X4A, 0x00);
	return 0;
}
...
static void es8396_i2c_shutdown(struct i2c_client *client)
{
	...
	#if 0
	snd_soc_write(tron_codec, 0X72, 0x00);
	snd_soc_write(tron_codec, 0X73, 0x00);	
	snd_soc_write(tron_codec, 0X4E, 0x84);
	snd_soc_write(tron_codec, 0X4F, 0x85);	
	snd_soc_write(tron_codec, 0x71, 0xfc);		
	snd_soc_write(tron_codec, 0X4A, 0x80);
	snd_soc_write(tron_codec, 0X4B, 0x80);		
	snd_soc_write(tron_codec, 0X52, 0xc1);		
	usleep_range(20000, 25000);
	es8396->aif1_select = 0;
	es8396->aif2_select = 0;
	es8396_set_bias_level(tron_codec, SND_SOC_BIAS_OFF);
	...
	#endif
        snd_soc_write(tron_codec, 0x70, 0xd4);
        //snd_soc_write(tron_codec, 0x71, 0x92);
       // snd_soc_write(tron_codec, 0x75, 0xc1);
	//usleep_range(200000, 500000);
       // snd_soc_write(tron_codec, 0x52, 0xc1);
	msleep(300);
        //snd_soc_write(tron_codec, 0x52, 0xc1);
	//snd_soc_write(tron_codec, 0X4B, 0x00);		
	//snd_soc_write(tron_codec, 0X4A, 0x00);
}
